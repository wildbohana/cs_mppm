...
namespace FTN.Services.NetworkModelService.TestClient
{
	public class Program
	{
		...
		{	
			...
			try
			{
				...
				do
				{
                    ...
                    // TODO dodaj opcije 4 i 5
                    else if (str == "4")
                    {
                        try
                        {
                            tgda.GetExtentValues(ModelCode.POWERTR);
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("Get all PowerTransformers failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
                    else if (str == "5")
                    {
                        try
                        {
                            List<long> ids = tgda.GetExtentValues(ModelCode.POWERTR);
                            long minId = ids.Min();
                            ...
                            tgda.GetRelatedValues(minId, association);
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("Get all TransformerWindings failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
		...
		private static void PrintMenu()
		{
            ...
            Console.WriteLine("\t4) Get PowerTransformers");
            Console.WriteLine("\t5) Get TransformerWindings");
